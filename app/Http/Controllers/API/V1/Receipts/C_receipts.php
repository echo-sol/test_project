...
// MODELS
use App\Http\Models\API\V1\Receipts\M_receipts as M_default;


class C_receipts extends Controller{

    private $module = "receipt";
    private $create_required = ['payment_id','status','created_by'];

/************* CREATE  *************/

    // [POST] api/receipt <-- Create new row
...
/************* GET/READ  *************/
    ...
    // [GET] api/receipts <-- Get all lists
    ...
    // [GET] api/receipt/{id} <-- Get specific row
    function get($id){
        $response = array();
...
/************* UPDATE  *************/
    ...
    // [PUT] api/receipt/{id} <-- Update specific row
    ...
    // [PUT] api/receipt/restore/{id} <-- Restore deleted specific row
    function restore($id){
        $response = array();
