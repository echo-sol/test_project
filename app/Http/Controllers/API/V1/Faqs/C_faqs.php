...
// MODELS
use App\Http\Models\API\V1\Faqs\M_faqs as M_default;


class C_faqs extends Controller{

    private $module = "faq";
    private $create_required = ['title','content','status','created_by'];

/************* CREATE  *************/

    // [POST] api/faq <-- Create new row
...
/************* GET/READ  *************/
    ...
    function list(Request $reqs){  
        ...
        foreach($faqs as $faqKey => $faq){
            ...
            $faqs[$faqKey]['contents'] = json_decode($contents);
        }
    ...
    // [GET] api/faq/{id} <-- Get specific row
    function get($id){
        $response = array();
...
/************* UPDATE  *************/
    ...
    // [PUT] api/faq/{id} <-- Update specific row
    ...
    // [PUT] api/faq/restore/{id} <-- Restore deleted specific row
    function restore($id){
        $response = array();
