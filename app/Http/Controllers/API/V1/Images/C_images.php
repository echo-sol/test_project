 ...
 * Description: Controller Images REST-API (CRUD)
**/
...
// MODELS
use App\Http\Models\API\V1\Images\M_images as M_default;


class C_images extends Controller{

    private $module = "image";
    private $create_required = ['url','status','created_by'];

/************* CREATE  *************/

    // [POST] api/image <-- Create new row
...
/************* GET/READ  *************/
    ...
    // [GET] api/images <-- Get all lists
        function list(Request $reqs){  
            ...
            $cdata->orderBy('m_images.image_id','DESC');
            $cdata = $cdata->get();
    ...
    // [GET] api/image/{id} <-- Get specific row
...
/************* UPDATE  *************/
    ...
    // [PUT] api/image/{id} <-- Update specific row
    ...
    // [PUT] api/image/restore/{id} <-- Restore deleted specific row
        function restore($id){
            $response = array();
...
/************* DELETE  *************/
    ...
    // [DELETE] api/image/{id} <-- SoftDelete specific row
    ...
    // [DELETE] api/image/delete/{id} <-- Permanent delete specific row
