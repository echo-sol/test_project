 ...
 * Description: Controller Downloadables Matches REST-API (CRUD)
**/
...
// MODELS
use App\Http\Models\API\V1\Downloadables\M_downloadable_matches as M_default;


class C_downloadable_matches extends Controller{

    private $module = "downloadable_match";
    private $create_required = ['downloadable_id','file_id','status','created_by'];

/************* CREATE  *************/

    // [POST] api/downloadable/match <-- Create new row (modified)
    function create($reqs){
        ...
        if($this->validation($this->create_required, $reqs)){
            ...
            $cdata = array();
            foreach($reqs->file_id as $file_id){
                $cdata[] = [
                    "file_id" => $file_id,
                    "downloadable_id" => $reqs->downloadable_id,
                    "status" => $reqs->status,
...
/************* GET/READ  *************/
    ...
    // [GET] api/downloadable/matches <-- Get all lists
    ...
    // [GET] api/downloadable/match/{id} <-- Get specific row
    /* function get($id){
        $response = array();
...
/************* UPDATE  *************/
    ...
    // [PUT] api/downloadable/match/{id} <-- Update specific row (modified)
    ...
    // [PUT] api/downloadable/match/restore/{id} <-- Restore deleted specific row (modified)
        function restore($id){
...
/************* DELETE  *************/
    ...
    // [DELETE] api/downloadable/match/{id} <-- SoftDelete specific row (modified)
    ...
    // [DELETE] api/downloadable/match/delete/{id} <-- Permanent delete specific row (modified)
