...
// MODELS
use App\Http\Models\API\V1\Users\M_user_role_access as M_default;


class C_user_role_access extends Controller{

    private $module = "user_role_access";
    private $create_required = ['user_role_id','urls','status','created_by'];

/************* CREATE  *************/

    // [POST] api/user/role/access <-- Create new row
...
/************* GET/READ  *************/
    ...
    // [GET] api/user/role/accesses <-- Get all lists
    ...
    // [GET] api/user/role/access/{id} <-- Get specific row
        function get($id){
            $response = array();

            $cdata = M_default::where('m_'.$this->module.'.'.$this->module.'_id', '=', $id)->first();
...
/************* UPDATE  *************/
    ...
    // [PUT] api/user/role/access/{id} <-- Update specific row
    ...
    // [PUT] api/user/role/access/restore/{id} <-- Restore deleted specific row
        function restore($id){
            $response = array();
...
/************* DELETE  *************/
    ...
    // [DELETE] api/user/role/access/{id} <-- SoftDelete specific row
    ...
    // [DELETE] api/user/role/access/delete/{id} <-- Permanent delete specific row
