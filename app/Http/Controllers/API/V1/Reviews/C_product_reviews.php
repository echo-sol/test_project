...
// MODELS
use App\Http\Models\API\V1\Reviews\M_product_reviews as M_default;


class C_product_reviews extends Controller{

    private $module = "product_review";
    private $create_required = ['review_id','product_id','status','created_by'];

/************* CREATE  *************/

    // [POST] api/product/review <-- Create new row
...
/************* GET/READ  *************/
    ...
    // [GET] api/review/products <-- Get all lists
        function list(Request $reqs){  
            $response = array();
            $paginate = ($reqs->input('paginate') == 'disable') ? false : true;
            $cdata = M_default::where('m_product_reviews.status','<>','0');
    ...
    // [GET] api/review/product/{id} <-- Get specific row
...
/************* UPDATE  *************/
    ...
    // [PUT] api/review/product/{id} <-- Update specific row
    ...
    // [PUT] api/review/product/restore/{id} <-- Restore deleted specific row
    function restore($id){
        $response = array();
...
/************* DELETE  *************/
    ...
    // [DELETE] api/review/product/{id} <-- SoftDelete specific row
    ...
    // [DELETE] api/review/product/delete/{id} <-- Permanent delete specific row
