 ...
 * Description: Controller States REST-API (CRUD)
**/
...
// MODELS
use App\Http\Models\API\V1\Worlds\M_states as M_default;


class C_states extends Controller{

    private $module = "state";
    private $create_required = ['name','has_city','status','created_by'];

/************* CREATE  *************/

    // [POST] api/state <-- Create new row
...
/************* GET/READ  *************/
    ...
    // [GET] api/state/{id} <-- Get specific row
    function get($id){
        $response = array();
...
/************* UPDATE  *************/
    ...
    // [PUT] api/state/{id} <-- Update specific row
    ...
    // [PUT] api/state/restore/{id} <-- Restore deleted specific row
    function restore($id){
        $response = array();
...
/************* DELETE  *************/
    ...
    // [DELETE] api/state/{id} <-- SoftDelete specific row
    ...
    // [DELETE] api/state/delete/{id} <-- Permanent delete specific row
